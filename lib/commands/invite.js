...
bot.command('invite', function(message) {
  invite(message).then(result => {
    let response = new Message().text("Успешно: " + result.success + "Всего: "+ result.all).to(message.from.id);
    bot.send(response);
    if(result.fails.length > 0) {
      let fails = result.fails.join(", \n"); 

      if(fails.length > 4095) {
        bot.send(new Message().text('Message to big watch logs.').to(message.from.id));
      } else {
        bot.send(new Message().text(fails).to(message.from.id));
      }
    }
  })
});
